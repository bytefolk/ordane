<!--
This repository holds one static page: no backend, no runtime dependencies, no
user data. The org-wide flow in bytefolk/.github still applies (issue → branch
→ PR → independent review → squash merge), but the evidence that used to be
written out by hand now runs in CI (.github/workflows/verify.yml): the contrast
audit, the render check, and a step that proves the render check still fails
closed. Point at the run instead of restating it.
-->
...
CI (`verify`) must be green. It runs, on every push:
